...
- `src/ingest.py`: PDF ingestion from `books/*.pdf` into Chroma using
  `OllamaEmbeddings(model="bge-m3")`, chunk size `1500`, overlap `300`.
- `src/rag.py`: LangChain Chroma retrievers, Gemma via Ollama, Spanish prompt
  templates, extract mode with exact-match prioritization, and JSON output
  contracts.
